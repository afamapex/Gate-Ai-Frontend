      ...
      const ffMat = new THREE.ShaderMaterial({
        ...
        fragmentShader:`#define MP 10
          ...
          void main(){
            ...
            float fr=pow(1.0-max(0.0,dot(n,viewDir)),3.2);
            float breathe=0.80+0.20*breath;\n            float lat=sin(vUv.y*6.0+time*0.7)*0.03;\n            vec3 col=baseColor*(fr*1.5+lat)*breathe;col+=baseColor*pow(fr,2.0)*0.7*breathe;\n\n            float alpha=(fr*0.82+lat*0.4)*breathe+pA*0.92;\n            if(pA>0.)col=mix(col,pC/pA,clamp(pA,0.,1.));gl_FragColor=vec4(col,clamp(alpha,0.,0.82));
          }`
      });
