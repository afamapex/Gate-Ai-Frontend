...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    function initGlobe() {
      ...
      // Force field
      const MP = 10;
      // Force field — wireframe shell (guaranteed visible) + subtle glow ring
      // Wireframe outer shell
      const ffWireGeo = new THREE.SphereGeometry(FF_R, 28, 18);
      const ffWireMat = new THREE.MeshBasicMaterial({
        color: 0x8866ff, wireframe: true, transparent: true, opacity: 0.22, depthWrite: false
      });
      scene.add(new THREE.Mesh(ffWireGeo, ffWireMat));

      // Soft glow rim (fresnel via ShaderMaterial)
      const ffU = {
      ...
      const ffMat = new THREE.ShaderMaterial({
        ...
        fragmentShader:`#define MP 10
          ...
          void main(){
            ...
            vec3 col=vec3(0.53,0.36,1.0)*fr*2.8*breathe;
            col+=vec3(0.3,0.55,1.0)*pow(fr,3.0)*1.6;
