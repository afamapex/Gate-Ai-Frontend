...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    function initGlobe() {
      ...
      const pl1 = new THREE.PointLight(0x7c63ff, 0.5, 20); pl1.position.set(-4, 3, 3); scene.add(pl1);
      const pl2 = new THREE.PointLight(0x20c7e8, 0.4, 20); pl2.position.set(4, -2, 2); scene.add(pl2);
      ...
      const fillLight = new THREE.DirectionalLight(0x9bd9ff, 0.28);
      ...
      const globe = new THREE.Mesh(
        ...
        new THREE.MeshPhongMaterial({map:earthTex, shininess:0, specular:new THREE.Color(0x000000), emissive:new THREE.Color(0x000000), emissiveIntensity:0.0})
      );
      ...
      // Force field
      const MP = 10;
      // Force field — pure fresnel glow only, no lines
      const ffU = {
        time:{value:0}, breath:{value:0}, camPos:{value:camera.position},
        pulsePos:{value:Array.from({length:10},()=>new THREE.Vector3())},
        pulseTimes:{value:new Array(10).fill(0)},
        pulseColors:{value:Array.from({length:10},()=>new THREE.Color(0xff4d6d))},
        pulseCount:{value:0},
      };
      const ffMat = new THREE.ShaderMaterial({
        transparent:true, side:THREE.FrontSide, depthWrite:false, uniforms:ffU,
        vertexShader:`varying vec3 vN,vPos,vWP;void main(){vN=normalize(normalMatrix*normal);vPos=position;vWP=(modelMatrix*vec4(position,1.)).xyz;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,
        fragmentShader:`#define MP 10
          uniform float time,breath;uniform vec3 camPos;
          uniform vec3 pulsePos[MP];uniform float pulseTimes[MP];uniform vec3 pulseColors[MP];uniform float pulseCount;
          varying vec3 vN,vPos,vWP;
          void main(){
            vec3 n=normalize(vN);vec3 viewDir=normalize(camPos-vWP);
            float fr=pow(1.0-max(0.0,dot(n,viewDir)),1.4);
            float breathe=0.82+0.18*breath;
            vec3 col=vec3(0.28,0.50,1.0)*fr*1.8*breathe;
            col+=vec3(0.18,0.72,1.0)*pow(fr,2.8)*1.2;
            float pA=0.0;vec3 pC=vec3(0.0);
            for(int i=0;i<MP;i++){if(float(i)>=pulseCount)break;float pt=pulseTimes[i];if(pt<=0.0||pt>=1.0)continue;float dist=acos(clamp(dot(normalize(vPos),normalize(pulsePos[i])),-1.0,1.0));float maxR=0.44;float ring=smoothstep(pt*maxR+0.03,pt*maxR,dist)*smoothstep(pt*maxR-0.10,pt*maxR,dist);float pa=ring*pow(1.0-pt,1.4)*4.0;pA+=pa;pC+=pulseColors[i]*pa;}
            float alpha=fr*1.6*breathe+pA*0.98;
            if(pA>0.0)col=mix(col,pC/pA,clamp(pA,0.0,1.0));
            gl_FragColor=vec4(col,clamp(alpha,0.0,0.62));
          }`
      });
      scene.add(new THREE.Mesh(new THREE.SphereGeometry(FF_R,64,64), ffMat));
  ...
  return (
    <>
        ...
        <div className="container hero-inner">
          ...
          <div className="hero-right">
            <div className="phone-bezel">
              <div className="phone-bezel-inner">
      ...
      {/* CAPABILITIES */}
      <section id="capabilities" style={{padding:'100px 0',backgroundImage:'url(/images/background/bg-capabilities.png)',backgroundSize:'auto 65%',backgroundPosition:'right center',backgroundRepeat:'no-repeat',backgroundColor:'#f9f8ff'}}>
        <div className="container">
...
section{position:relative;overflow:hidden}.eyebrow{display:inline-flex;align-items:center;gap:10px;padding:9px 16px;border:1px solid rgba(108,92,231,.34);background:rgba(255,255,255,.74);border-radius:999px;color:var(--accent);font-weight:800;font-size:12px;letter-spacing:2px;text-transform:uppercase;box-shadow:0 12px 35px rgba(108,92,231,.12);backdrop-filter:blur(10px)}.eyebrow-dot{width:7px;height:7px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--cyan));box-shadow:0 0 12px rgba(108,92,231,.7),0 0 24px rgba(37,199,232,.4);animation:dotpulse 2.4s ease infinite}@keyframes dotpulse{0%,100%{box-shadow:0 0 12px rgba(108,92,231,.7),0 0 24px rgba(37,199,232,.4)}50%{box-shadow:0 0 18px rgba(108,92,231,1),0 0 32px rgba(37,199,232,.6)}}.h-display{font-size:clamp(42px,4.2vw,76px);line-height:.98;letter-spacing:-.055em;font-weight:800;color:#070b24}.h-section{font-size:clamp(40px,4.4vw,74px);line-height:1.02;letter-spacing:-.055em;font-weight:800;color:#070b24}.lede{font-size:clamp(16px,1.3vw,21px);line-height:1.65;color:var(--text-2);max-width:700px}.accent{background:linear-gradient(125deg,#6C5CE7 0%,#2D8CFF 52%,#20c8df 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:transparent}

.hero{min-height:100svh;padding:150px 0 78px;background:url('/images/background/bg-hero.png') center center/cover no-repeat;display:flex;align-items:center;}.hero::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(255,255,255,.30) 0%,rgba(255,255,255,.06) 50%,rgba(255,255,255,.18) 100%);pointer-events:none}.section-overlay{position:relative;z-index:2}.section-overlay::before{content:'';position:absolute;inset:-16px -20px;border-radius:16px;background:rgba(255,255,255,.72);backdrop-filter:blur(4px);z-index:-1;pointer-events:none}.hero-globe-canvas{position:absolute;inset:0;width:100%;height:100%;z-index:1;pointer-events:none;opacity:.92;mix-blend-mode:normal}.hero-inner{display:grid;grid-template-columns:minmax(380px,480px) 1fr;gap:56px;align-items:center;min-height:calc(100svh - 220px);position:relative;z-index:4}.hero-left{position:relative;z-index:5;max-width:500px}.hero-left .eyebrow{margin-bottom:28px}.hero-left .h-display{margin-bottom:28px}.hero-left .accent{font-style:italic;font-weight:800}.hero-lede{font-size:clamp(15px,1.1vw,18px);line-height:1.55;color:#273254;max-width:710px;margin:0 0 36px}.hero-lede br+*{color:inherit}.hero-ctas{display:flex;gap:16px;align-items:center;flex-wrap:wrap}.hero-right{position:relative;z-index:6;display:flex;justify-content:flex-end;align-items:center}.hero-globe-counter{position:absolute;left:51%;bottom:8.4%;transform:translateX(-50%);z-index:5;display:flex;align-items:center;gap:16px;padding:14px 22px;border-radius:18px;background:rgba(255,255,255,.82);border:1px solid rgba(200,210,235,.7);box-shadow:0 18px 40px rgba(40,60,120,.12);backdrop-filter:blur(14px)}.hgc-item{display:flex;align-items:center;gap:7px;font-size:14px;font-weight:800;color:#253052}.hgc-dot{width:9px;height:9px;border-radius:50%;display:inline-block;box-shadow:0 0 14px currentColor}.hgc-red{background:var(--danger);color:var(--danger)}.hgc-green{background:var(--green);color:var(--green)}.hgc-item + .hgc-item{border-left:1px solid #dce3f2;padding-left:16px}

.phone-shell{width:min(100%,420px);padding:0;border-radius:48px;background:transparent;box-shadow:0 30px 90px rgba(108,92,231,.22),0 8px 24px rgba(35,49,95,.14),0 0 0 2px rgba(108,92,231,.15)}.phone-bezel{position:relative;padding:10px;border-radius:56px;background:linear-gradient(145deg,rgba(255,255,255,.9),rgba(235,232,255,.95));box-shadow:0 0 0 1.5px rgba(108,92,231,.25),0 30px 80px rgba(108,92,231,.2),inset 0 1px 0 rgba(255,255,255,.9)}.phone-bezel-inner{position:absolute;inset:0;border-radius:56px;background:linear-gradient(145deg,rgba(255,255,255,.2),transparent 50%);pointer-events:none;z-index:10}.phone-notch{width:80px;height:10px;background:rgba(108,92,231,.18);border-radius:0 0 12px 12px;margin:0 auto 0;position:relative;z-index:11}.phone-frame{background:#f8f7ff;border:1px solid rgba(108,92,231,.18);border-radius:44px;overflow:hidden;backdrop-filter:blur(20px)}.pf-top{height:44px;display:flex;align-items:center;gap:8px;padding:0 18px;border-bottom:1px solid rgba(108,92,231,.14);background:linear-gradient(135deg,rgba(108,92,231,.12),rgba(125,107,255,.08))}.mac-dot{width:12px;height:12px;border-radius:50%}.mac-r{background:#ff5f57;box-shadow:0 0 6px rgba(255,95,87,.4)}.mac-y{background:#febc2e;box-shadow:0 0 6px rgba(254,188,46,.4)}.mac-g{background:#28c840;box-shadow:0 0 6px rgba(40,200,64,.4)}.pf-bar-right{margin-left:auto;display:flex;gap:8px;align-items:center}.pf-label{font-family:var(--mono);font-size:10px;color:#8a95ad}.pf-label b{color:var(--accent)}.pf-body{height:540px;position:relative;overflow:hidden;background:linear-gradient(180deg,#f5f3ff 0%,#eceaff 50%,#f0f4ff 100%)}.ph{position:absolute;inset:0;transition:opacity .4s;display:flex;flex-direction:column}.ph-ring,.ph-done{align-items:center;justify-content:center}.ph-done,.ph-chat{opacity:0}.ring-bg{position:absolute;inset:0;background:radial-gradient(ellipse at center,rgba(108,92,231,.22) 0%,rgba(108,92,231,.08) 40%,transparent 68%)}.ring-content{position:relative;display:flex;align-items:center;flex-direction:column;gap:14px}.ring-wrap{width:64px;height:64px;position:relative;display:grid;place-items:center}.rp{position:absolute;inset:0;border-radius:50%;border:1.4px solid rgba(108,92,231,.26);animation:rout 2s ease infinite}.rp:nth-child(2){animation-delay:.65s}.rp:nth-child(3){animation-delay:1.3s}@keyframes rout{0%{transform:scale(.72);opacity:.9}100%{transform:scale(2.4);opacity:0}}.ring-icon{width:42px;height:42px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(135deg,#7d6bff,#2D8CFF);box-shadow:0 16px 40px rgba(108,92,231,.45),0 0 0 6px rgba(108,92,231,.12);animation:rbob .45s ease infinite alternate}.ring-icon svg{stroke:#fff!important}@keyframes rbob{from{transform:rotate(-7deg)}to{transform:rotate(7deg)}}.ring-lbl,.done-lbl{font-size:13px;color:#596482;font-weight:700}.done-circle{width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#10c99b,#23d3e2);display:grid;place-items:center;transform:scale(0);transition:transform .45s cubic-bezier(.34,1.56,.64,1);box-shadow:0 18px 42px rgba(16,201,155,.24)}.done-lbl{opacity:0;margin-top:14px;transition:opacity .3s .2s}.chat-tabs{display:flex;gap:6px;padding:12px 12px 8px}.c-tab{flex:1;border:1px solid #dfe5f3;background:rgba(255,255,255,.62);color:#7b86a0;padding:10px 8px;border-radius:11px;font-weight:800;font-size:13px;cursor:pointer}.c-tab.active{border-color:#8d7cff;background:linear-gradient(180deg,rgba(108,92,231,.12),rgba(255,255,255,.65));color:var(--accent)}.chat-hdr{padding:0 12px 8px;border-bottom:1px solid #e6ebf6}.hdr-pill{display:inline-flex;align-items:center;gap:5px;border:1px solid #dde5f4;background:#fff;border-radius:999px;padding:5px 10px;font-size:11px;font-weight:800;color:#2d3856}.hdr-live{width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 12px var(--green);animation:livep 1.4s ease infinite}@keyframes livep{50%{opacity:.35;box-shadow:none}}.msgs-wrap{position:relative;flex:1;overflow:hidden}.msgs{position:absolute;inset:0;overflow:auto;padding:12px;display:flex;flex-direction:column;gap:12px;transition:opacity .45s}.msgs::-webkit-scrollbar{display:none}.chat-msg{display:flex;flex-direction:column;gap:4px;opacity:0;transform:translateY(8px);transition:.35s}.chat-msg.show{opacity:1;transform:none}.chat-msg.gate{align-self:flex-start;max-width:88%}.chat-msg.caller{align-self:flex-end;max-width:88%}.chat-msg-hdr{display:flex;align-items:center;gap:6px}.chat-msg.caller .chat-msg-hdr{flex-direction:row-reverse}.chat-gate-av,.chat-caller-av{width:18px;height:18px;border-radius:50%;display:grid;place-items:center;flex:0 0 auto}.chat-gate-av{background:linear-gradient(135deg,#6C5CE7,#2D8CFF)}.chat-caller-av{background:#fff;border:1px solid #dce4f5}.chat-msg-who{font-size:9px;letter-spacing:.6px;text-transform:uppercase;font-weight:900}.chat-msg.gate .chat-msg-who{color:var(--accent)}.chat-msg.caller .chat-msg-who{color:#8b96b2}.chat-bubble{padding:10px 13px;border-radius:14px;font-size:13px;line-height:1.44;color:#1c2745;background:#fff;border:1px solid #e1e7f5;box-shadow:0 10px 28px rgba(40,55,110,.08)}.chat-msg.gate .chat-bubble{background:linear-gradient(135deg,#f0ecff,#ebe6ff);border-color:#cdc4ff;border-bottom-left-radius:4px;box-shadow:0 4px 12px rgba(108,92,231,.1)}.chat-msg.caller .chat-bubble{background:#f7f9ff;border-bottom-right-radius:4px}.chat-typing{width:max-content;padding:12px 15px;background:#fff;border:1px solid #e0e7f5;border-radius:16px;display:flex;gap:4px;opacity:0}.chat-typing.show{opacity:1}.chat-typing span{width:5px;height:5px;border-radius:50%;background:#8a95ad;animation:td 1.2s ease infinite}.chat-typing span:nth-child(2){animation-delay:.15s}.chat-typing span:nth-child(3){animation-delay:.3s}@keyframes td{50%{transform:translateY(-4px);opacity:.5}}.result-takeover{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:24px;opacity:0;pointer-events:none;transition:opacity .35s;background:linear-gradient(180deg,rgba(255,255,255,.96),rgba(245,248,255,.96))}.result-takeover.show{opacity:1}.shield-big{transform:scale(.8);transition:.35s}.shield-big.pop{transform:scale(1)}.result-title{font-weight:900;font-size:17px;margin:0;opacity:0;transition:.3s;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}.result-title.show,.result-sub.show{opacity:1}.result-title.blocked{color:var(--danger)}.result-title.forwarded{color:var(--green)}.result-sub{font-size:12px;color:#65718f;opacity:0;margin:0;text-align:center;line-height:1.5}
...
@media(max-width:720px){.nav-inner{padding:0 14px;border-radius:18px;height:62px}.nav-links{display:none}.nav-cta{gap:8px}.nav-cta .btn-ghost{display:flex!important;padding:6px 12px!important;font-size:12px!important;white-space:nowrap!important}.nav-btn{height:38px!important;padding:0 14px!important;font-size:12px!important}.hero{padding:88px 0 0;min-height:100svh;display:block;position:relative;overflow:hidden}.hero-globe-canvas{position:absolute!important;top:40%!important;left:50%!important;transform:translate(-50%,-50%)!important;width:130vw!important;height:130vw!important;opacity:.35!important;z-index:0!important}.hero-inner{display:block!important;position:relative;z-index:5;padding:20px 16px 0}.hero-left{background:rgba(255,255,255,.86);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-radius:20px;padding:22px 18px 20px;margin-bottom:16px}.hero-right{display:none!important}.h-display{font-size:clamp(32px,8.5vw,44px)!important;line-height:1.03!important;letter-spacing:-.04em!important}.hero-lede{font-size:13px!important;line-height:1.52!important;margin:10px 0 16px!important}.hero-ctas{flex-direction:row!important;flex-wrap:wrap!important;gap:8px!important}.hero-ctas .btn{font-size:12.5px!important;padding:10px 16px!important}.hero-globe-counter{position:relative!important;left:auto!important;bottom:auto!important;transform:none!important;margin:0 auto!important;display:table!important;font-size:11.5px;padding:8px 14px}.stats-panel{grid-template-columns:1fr}.stat-cell{border-right:0!important;border-bottom:1px solid #dbe3f4!important;padding:22px 18px}.stat-cell .stat-icon-circle{position:relative!important;top:auto!important;left:auto!important;margin-bottom:10px}.stat-cell .stat-viz{display:block!important;opacity:.12!important}.stat-cell .stat-num{font-size:clamp(40px,11vw,58px)!important;margin-bottom:6px!important}.stat-cell:last-child{border-bottom:0!important}.problem,.stats,#capabilities,#pricing,#faq,.industries,.integrations,.cta{padding:56px 0!important}.problem-grid,.caps-grid,.pricing-grid{grid-template-columns:1fr!important;gap:14px!important}.problem-card,.cap{padding:22px!important;min-height:auto!important}.industry-chip{width:100%!important;justify-content:center!important}.integrations-grid{gap:10px!important}.ind-stats{grid-template-columns:1fr!important}.faq-item summary{font-size:14px!important;padding:14px 16px!important}.faq-item p{padding:0 16px 14px!important;font-size:13px!important}.h-section{font-size:clamp(26px,7.5vw,38px)!important}.pricing-grid{grid-template-columns:1fr!important}.phone-section{display:block}.phone-shell{width:min(100%,300px)!important;margin:0 auto!important}.phone-bezel{padding:7px!important;border-radius:44px!important}.phone-frame{border-radius:36px!important}}
