      ...
      const ffMat = new THREE.ShaderMaterial({
        ...
        fragmentShader:`#define MP 10
          ...
          void main(){
            ...
            float fr=pow(1.0-max(0.0,dot(n,viewDir)),2.2);
            float breathe=0.80+0.20*breath;\n            float lat=sin(vUv.y*6.0+time*0.7)*0.03;\n            vec3 col=baseColor*(fr*2.2+lat)*breathe;col+=baseColor*pow(fr,1.4)*1.1*breathe;col+=vec3(0.4,0.35,1.0)*pow(fr,4.0)*1.8;\n\n            float alpha=(fr*1.2+lat*0.3)*breathe+pA*0.95;\n            if(pA>0.)col=mix(col,pC/pA,clamp(pA,0.,1.));gl_FragColor=vec4(col,clamp(alpha,0.,0.72));
          }`
      });
